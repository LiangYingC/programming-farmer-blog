...
由於我所在的公司可以用很低的成本使用 GCP Cloud 的大多服務，加上後端已經有用 GCP Cloud Logging，讓技術棧統一也比較好管理，所以當時就選擇用 GCP Cloud Logging，在此也會以 GCP Cloud Logging 作為簡要示範。
...
選擇 GCP Monitoring 的原因，是和 GCP Logging 類似，就是有低成本的額度跟技術棧的統一。

選擇 Slack 的原因，則是它與各大 APP 串接整合的很好，通常介面上點一點即可。
